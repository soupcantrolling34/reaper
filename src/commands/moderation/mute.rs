                ...
                match Value::to_string(&option.value.clone().unwrap()).replace("\"", "").parse::<i64>() {
                    Ok(id) => {
                        if id == cmd.user.id.0 as i64 {
                            ...
                            return send_message(&ctx, &cmd, "You cannot mute yourself".to_string(), Some(true)).await;
                        }
                        user_id = Some(id)
                    },
                    Err(err) => {
    ...
    match handler.mute(
        ...
        Ok(action) => {
            if let Some(action) = action {
                ...
                return send_message(&ctx, &cmd, format!("Failed to mute <@{}> because there is no mute role configured", user_id.unwrap()), None).await;
            }
        },
        Err(_) => {
