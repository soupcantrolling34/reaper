...
use crate::{Handler, commands::{structs::CommandError, utils::{messages::send_message}}, mongo::structs::{Action, ActionType, Permissions}};
                ...
                match Value::to_string(&option.value.clone().unwrap()).replace("\"", "").parse::<i64>() {
                    Ok(id) => {
                        if id == cmd.user.id.0 as i64 {
                            ...
                            return send_message(&ctx, &cmd, "You cannot kick yourself".to_string(), Some(true)).await;
                        }
                        user_id = Some(id)
                    },
                    Err(err) => {
    ...
    match handler.kick(
        ...
        Ok(action) => {
            if let Some(action) = action {
                ...
                return send_message(&ctx, &cmd, "Failed to kick user. Please try again later.".to_string(), Some(true)).await;
            }
        },
        Err(err) => {
