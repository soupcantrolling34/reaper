...
pub async fn user_run(handler: &Handler, ctx: &Context, cmd: &ApplicationCommandInteraction) -> Result<(), CommandError> {
                    ...
                    Some(perm) => {
                        match Permissions::from(perm.to_string()) {
                            Permissions::Unknown => {
                                ...
                                return send_message(&ctx, &cmd, format!("`{}` is not a valid permission and could not be removed", perm), Some(true)).await;
                            }
    ...
    match handler.mongo.get_user(
        ...
        Ok(user) => {
            if !user.permissions.contains(&permission.unwrap()) {
                ...
                return send_message(&ctx, &cmd, format!("<@{}> does not have `{}`", user_id.unwrap(), permission.unwrap().to_string()), Some(true)).await;
            }
        },
        Err(err) => {
    ...
    match handler.mongo.remove_permission_from_user(
        user_id.unwrap(),
        cmd.guild_id.unwrap().0 as i64,
        permission.unwrap()
    ).await {
        Ok(_) => {
            send_message(&ctx, &cmd, format!("Successfully removed `{}` from <@{}>", permission.unwrap().to_string(), user_id.unwrap()), None).await
        },
        Err(err) => {
...
pub async fn role_run(handler: &Handler, ctx: &Context, cmd: &ApplicationCommandInteraction) -> Result<(), CommandError> {
    ...
    match handler.mongo.get_role(
        ...
        Ok(role) => {
            if !role.permissions.contains(&permission.unwrap()) {
                ...
                return send_message(&ctx, &cmd, format!("<@&{}> does not have `{}`", role_id.unwrap(), permission.unwrap().to_string()), Some(true)).await;
            }
        },
        Err(err) => {
    ...
    match handler.mongo.remove_permission_from_role(
        role_id.unwrap(),
        cmd.guild_id.unwrap().0 as i64,
        permission.unwrap()
    ).await {
        Ok(_) => {
            send_message(&ctx, &cmd, format!("Successfully removed `{}` from <@&{}>", permission.unwrap().to_string(), role_id.unwrap()), None).await
        },
        Err(err) => {
